...
def main(args):
    ...
    dt = pd.read_csv("/home/danli/workspace/pretrain/mae_zwy/downstream/chronicdisease5/"+args.data_name+'.csv')
    import math
...
if __name__ == '__main__':
    ...
    #args.output_dir = os.path.join(args.output_dir, 'chronicdisease5', "loop"+current_time.strftime("%Y-%m-%d-%H%M"))
    ...
    tasks=['MI', 'stroke',  'dem', 'PD']
    for i in range(args.test_num):
        args.seed = i
        for task in tasks:
            args.data_name = task
            args.output_dir = os.path.join(or_dir, "seed"+str(i), args.data_name, "epoch"+str(args.now_epoch))
